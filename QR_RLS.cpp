...
#include "QR_RLS.h"
#include "add_row_col.cpp"

using namespace std;

QR_Rls::QR_Rls(double *x, double *y, int max_obs, double ff, double lambda, int dim, int X_rows)
    : X(x), y(y), max_obs(max_obs), l(lambda), n_batch(X_rows), n_obs(X_rows), dim(dim)
{
    ...
    auto [all_Q, R] = Q_R_compute(this, X, X_rows, dim);
