#include "baselines/KRLS_RBF/krls_rbf_test.h"
...
int main()
{
   ...
   KRLS_RBF k_rls(X, y, num_rows, num_cols, lambda, sigma, num_rows);
   ...
   for (int i = 0; i < n_its; i++)
   {
      ...
      k_rls.update(X_update, y_update[i], pred, eps_post);

      // preds.push_back(k_rls.pred(X_update));
